...
// Directly provide the OpenAI API key
...
app.post('/sentiment-analysis', async (req, res) => {
    ...
    try {
        ...
        const mood = analysis[0]?.mood || 'Neutral'; // Assuming first face detected
        console.log('Detected Mood:', mood);

        // Fetch movie suggestions from OMDb API
        const movieSuggestions = await getMovieSuggestions(mood);
        console.log('Movie Suggestions:', movieSuggestions);

        // Return sentiment analysis and movie suggestions
        console.log('Sentiment Analysis Result:', analysis);
        return res.status(200).json({ analysis, movieSuggestions });
    } catch (error) {
...
// Helper function to map detected emotions to a mood
...
// Helper function to fetch movie suggestions from OMDb API
// Helper function to fetch movie suggestions from OMDb API based on genre
const getMovieSuggestions = async (mood) => {
    const moodToGenre = {
        Happy: 'Comedy',
        Sad: 'Drama',
        Angry: 'Action',
    ...
    const genre = moodToGenre[mood] || 'Drama'; // Default to Drama if mood isn't mapped
    const omdbUrl = `http://www.omdbapi.com/?apikey=${OMDB_API_KEY}&s=${encodeURIComponent(genre)}&type=movie`;

    try {
        console.log(`Fetching movie suggestions from OMDb API for genre: ${genre}`);
        ...
        // Format the movie suggestions with IMDb links
        ...
        console.error('Error fetching movie suggestions from OMDb API:', error.message);
        return [];
    }
};
